...
이번에 제가 풀었던 문제 중 상대적으로 간단한 두번째 문제입니다. (예제 그림만 봐도 대략 이해가 됩니다.)
...
간단하게 공백판을 만들고 다이아몬드의 크기를 계산해서 각 4분면에 *를 표시한 후 이를 출력하는 코드입니다.
...
코드골프는 회사에서는 절대 하면 안되는 여러가지 흑마술(?)을 하게 되기 때문에 하지 말라고 하는 일을 하는 쾌감이 있습니다. 그리고 제가 [파이콘 2018 도도 파이터 후기](https://brunch.co.kr/@leehosung/50)에서 이야기 한 것처럼 이런 코딩 경기에 참여하는 것은 컨퍼런스를 즐기는 또 다른 방법이자, 개발자들만 할 수 있는 놀이입니다. 이 글을 통해 코드 골프라는 것이 대단하지 않은것이구나라는 것을 알게 되지 않으셨나요? 여러분도 다음기회에 도전해 보세요. 
